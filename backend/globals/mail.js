...
function sendEmailConfirmation(username, email, token) {
  return new Promise( (resolve, reject) => {
    if(process.env.NODE_ENV != 'production') {
      ...
      console.log('CANCEL REGISTRATION: \n', `http://localhost:9000/cancel/${token}`)
      return resolve()
    }
    ...
    fs.promises.readFile(path.resolve(__dirname, '../emails/emailConfirmation.html'), 'utf8')
      .then( html => {
        ...
        html = html.replace(/\{\{CONFIRM_URL\}\}/g, `https://speer.fun/confirm/${token}`)
        html = html.replace(/\{\{CANCEL_URL\}\}/g, `https://speer.fun/cancel/${token}`)
